...
A __matrix__ is a vector with two additional attributes: the number of rows and the number of columns. Since matrices are vectors, they also have modes, such as numeric and character. (On the other hand, vectors are not one-column or one-row matrices.). Matrices are special cases of a more general R type of object: __arrays__. Arrays can be multidimensional. For example, a three-dimensional array would consist of rows, columns, and layers, not just rows and columns as in the matrix case. Much of R’s power comes from the various operations you can perform
on matrices.

## Creating Matrices

Matrix row and column subscripts begin with 1. The internal storage of a matrix is in column-major order, meaning that first all of column 1 is stored, then all of column 2, and so on. The fact that R uses column-major order then determines where these four numbers are put within the matrix. We do need to warn R ahead of time that y will be a matrix and give the number of rows and columns. Though internal storage of a matrix is in column-major order, you can set the byrow argument in matrix() to true to indicate that the data is coming in row-major order.
...
Filtering can be done with matrices, just as with vectors. The filtering criterion can be based on a variable separate from the one to which the filtering will be applied.
...
- `m` is the matrix.
- `dimcode` is the dimension, equal to __1__ if the function applies to __rows__ or __2__ for __columns__.
- `f` is the function to be applied.
- `fargs` is an optional set of arguments to be supplied to `f`.

A function you write yourself is just as legitimate for use in `apply()` as any R built-in function such as `mean()`. If the function to be applied returns a vector of _k_ components, then the result of `apply()` will have _k_ rows. You can use the matrix __transpose__ function `t()` to change it if necessary. If the function returns a scalar (which we know is just a one-element vector), the final result will be a vector, not a matrix. The function to be applied needs to take at least one argument. The formal argument here will correspond to an actual argument of one row or column in the matrix, as described previously. In some cases, you will need additional arguments for this function, which you can place following the function name in your call to `apply()`.
...
The `rbind()` (row bind) and `cbind()` (column bind) functions let you add rows or columns to a matrix. Be careful with `rbind` and `cbin()`, though. Like creating a vector, creating a matrix is time consuming (matrices are vectors, after all).

```{r}
one <- c(1, 1, 1, 1)
z <- matrix(c(1, 1, 1, 1, 1, 2, 3, 4, 1, 1, 0, 0, 1, 0, 1, 0), nrow = 4, ncol = 4)
z
z <- cbind(one, z)
cbind(1, z)
...
In the world of statistics, dimension reduction is a good thing, with many statistical procedures aimed to do it well. If we are working with, say, 10 variables and can reduce that number to 3 that still capture the essence of our data, we’re happy. However, in R, something else might merit the name dimension reduction that we may sometimes wish to avoid. This seems innocuous, but note the format in which R has displayed r. It’s a vector format, not a matrix format. In other words, r is a vector of length 2, rather than a 1-by-2 matrix. This seems natural, but in many cases, it will cause trouble in programs that do a lot of matrix operations. You may find that your code works fine in general but fails in a special case. For instance, suppose that your code extracts a submatrix from a given matrix and then does some matrix operations on the submatrix. If the submatrix has only one row, R will make it a vector, which could ruin your computation. Fortunately, R has a way to suppress this dimension reduction: the `drop` argument. If you have a vector that you wish to be treated as a matrix, you can use the `as.matrix()`.

```{r}
z <- matrix(c(1, 2, 3, 4, 5, 6, 7, 8), nrow = 4, ncol = 2)
z
r <- z[2, ]
r
attributes(z)
...
The natural way to refer to rows and columns in a matrix is via the row and column numbers. However, you can also give names to these entities. The function `rownames()` works similarly. Naming rows and columns is usually less important when writing R code for general applications, but it can be useful when analyzing a specific data set.
...
# subscripting elements in an array
tests[3, 2, 1]
```
...
If we don’t want the default value of a given function, we must name the argument in our call. Hence the terminology _named argument_. In the followed example `header = TRUE` would be the named argument.
...
### Deciding Whether to Explicitly Call _return()_
...
Since the return value can be any R object, you can return complex objects. If your function has multiple return values, place them in a list or other container.
...
R functions are _first-class objects_ (of the class "function", of course), meaning that they can be used for the most part just like other objects. Even the "{" is a function. Arguments to function() can later be accessed via the R functions `formals()` and `body()`. Using R in interactive mode, simply typing the name of an object results in printing that object to the screen. Functions are no exception, since they are objects just like anything else. If you wish to view a lengthy function in this way, run it through `page()`. Some of R’s most fundamental built-in functions are written directly in C, and thus they are not viewable in this manner.

Since functions are objects, you can also assign them, use them as arguments to other functions, and so on. And since functions are objects, you can loop through a list consisting of several functions. This would be useful, for instance, if you wished to write a loop to plot a number of functions on the same graph.
...
A function consists not only of its arguments and body but also of its environment. The latter is made up of the collection of objects present at the time the function is created.

### The Top-Level Environment

The function `f()` is created at the top level—that is, at the interpreter command prompt—and thus has the top-level environment, which in R output is referred to as `R_GlobalEnv` but which confusingly you refer to in R code as `.GlobalEnv`. The function `ls()` lists the objects of an environment. If you call it at the top level, you get the top-level environment.
